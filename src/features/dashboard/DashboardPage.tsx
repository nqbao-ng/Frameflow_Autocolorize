...
export function Dashboard() {
  ...
  const {
    ...
    detachedReferenceFrameId, showReferencePreview, setShowReferencePreview,
  ...
  const activeFrameUsesDetachedReference = Boolean(
    activeFrameData && activeFrameData.id === detachedReferenceFrameId,
  );
  const activeLocalPaint = frameStates[activeFrame] === "manual"
    ? framePaints[activeFrame]
    : null;
  const activePaintUrl = activeLocalPaint
    || (!activeFrameUsesDetachedReference ? activeFrameData?.paintUrl || null : null);
          ...
          <div style={{ flex: 1, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", background: "#0F172A", position: "relative", minHeight: 0, borderRadius: 14, overflow: "hidden" }}>
            {showReferencePreview && referenceImage?.paintUrl ? (
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#0F172A" }}>
                ...
                <button
                  ...
                  style={{ position: "absolute", top: 12, left: 12, width: 30, height: 30, borderRadius: 8, border: "1px solid rgba(255,255,255,0.18)", background: "rgba(15,23,42,0.72)", color: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                >
                  <X size={14} />
                </button>
              </div>
              ...
              <PaintCanvas
                ref={paintCanvasRef}
                imageUrl={activeFrameData?.url ?? null}
                paintUrl={activePaintUrl}
                tool={activeTool}
                color={activeColor}
                brushSize={brushSize}
                opacity={opacity}
            ...
            <div style={{ position: "absolute", bottom: 12, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", borderRadius: 100, padding: "4px 12px", pointerEvents: "none" }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: "white" }}>
                {showReferencePreview ? "Reference Preview" : `Frame ${activeFrame + 1} of ${uncoloredFiles.length || 0}`}
              </span>
            </div>
            ...
            {referenceImage && (
              <button
                ...
                style={{ position: "absolute", top: 12, right: 12, background: showReferencePreview ? "rgba(59,130,246,0.9)" : "rgba(0,0,0,0.55)", backdropFilter: "blur(8px)", borderRadius: 9, padding: "4px 9px", display: "flex", alignItems: "center", gap: 6, border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer" }}
              >
                <img src={referenceImage.paintUrl || referenceImage.url} alt="ref" style={{ width: 22, height: 22, borderRadius: 4, objectFit: "cover" }} />
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: 8, color: "rgba(255,255,255,0.65)", marginBottom: 1 }}>Reference · click to view</div>
            ...
            {/* AI colored badge */}
            {!showReferencePreview && frameStates[activeFrame] === "ai" && (
            ...
            {/* Manual edit badge */}
            {!showReferencePreview && frameStates[activeFrame] === "manual" && (
