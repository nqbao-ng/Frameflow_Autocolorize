...
export async function enforceCreativeLimits(supabase, userId) {
  const maxActive = Math.max(1, Number(process.env.FRAMEFLOW_CREATIVE_MAX_ACTIVE_PER_USER) || 2);
  const dailyLimit = Math.max(1, Number(process.env.FRAMEFLOW_CREATIVE_DAILY_LIMIT) || 30);
  ...
  if ((activeCount || 0) >= maxActive) {
    const error = new Error(`You already have ${activeCount} active Creative Studio job(s). Wait for one to finish.`);
  ...
  if ((dailyCount || 0) >= dailyLimit) {
    const error = new Error(`Daily Creative Studio limit reached (${dailyLimit} jobs).`);
    error.statusCode = 429;
    throw error;
  }
}
